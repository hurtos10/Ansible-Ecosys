...
$InstallCommand = "scoop install main/maven"

# Ejecutar el comando Scoop para instalar Gradle
Write-Host "Iniciando la instalación de maven usando Scoop..." -ForegroundColor Yellow
try {
    scoop install main/maven
    if ($LASTEXITCODE -ne 0) {
        Write-Error "La instalación de maven falló."
        exit 1
    }
    Write-Host "La instalación de maven se ha completado." -ForegroundColor Green
} catch {
